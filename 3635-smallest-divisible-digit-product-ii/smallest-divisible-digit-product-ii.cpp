...
private:
    ...
    string buildSuffix(vector<int> primeCount, int targetLength, string result) {
    ...
    int getMinLength(vector<int>& primeCount) {
        ...
        return (count3 / 2)+ (count2 / 3)+ max(0, primeCount[7])+ max(0, primeCount[5]) + (count23 == 3 ? 2 : (count23 > 0 ? 1 : 0));
