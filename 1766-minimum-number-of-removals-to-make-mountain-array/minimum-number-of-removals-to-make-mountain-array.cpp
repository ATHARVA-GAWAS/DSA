...
public:
    ...
    int minimumMountainRemovals(vector<int>& nums) {
        ...
        //iterate over all i and minimum 
        for(int i=1;i<n-1;i++){
            //finding subsequence left side of i which is decreasing
            int left=Leftsolve(nums,i-1,i,dpleft);  
            //finding subsequence right side of i which is increasing
            int right=rightsolve(nums,i+1,i,dpright); 
            //left ans right both should at least one
            if(left&&right){
